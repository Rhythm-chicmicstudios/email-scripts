...
# Add a dictionary to track the counts of each status
status_counts = {
    "valid": 0,
    "invalid": 0,
    "unknown": 0,
...
def is_accept_all(domain, logger):
    ...
    try:
        ...
        logger.debug(f"Checking accept-all for domain '{domain}' using MX host '{mx_host}'.")
        ...
        logger.warning(f"Unable to connect to mail server for domain: {domain} with MX host: {mx_host}")
        return None
    except Exception as e:
        
        logger.error(f"Error checking accept-all for {domain}: {e},  with code {code}")
        return None
...
def verify_email(email, logger):
    ...
    try:
        ...
        # No MX Records Exist: The domain does not have any MX records configured.
        # Without MX records, the domain cannot receive emails directly since MX records define the mail servers responsible for receiving email messages for a domain.
...
# At the end of the main function, log the summary table
def log_status_counts(logger):
    logger.info("\nStatus Summary:")
...
def validate_args(args, logger):
    if len(args) < 7:
        logger.error("Missing arguments. You need to provide at least 7 arguments for DB connection.")
...
def main():
    ...
    try: 
        ...
        # Log the summary of email status counts
        log_status_counts(logger)
