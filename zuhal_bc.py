...
from asyncio import sleep, CancelledError, create_task
...
def check_domain(domain, logger):
    try:
        dns.resolver.resolve(domain, 'MX', lifetime=20)
        logger.debug(f"Domain '{domain}' has MX records.")
        return True
    except (dns.resolver.NXDOMAIN, dns.resolver.NoAnswer, dns.resolver.Timeout):
        try:
            dns.resolver.resolve(domain, 'A', lifetime=20)
...
def update_task_status(db_connection, task_id, logger):
    try:
        cursor = db_connection.cursor()
        update_query = """
            UPDATE tasks
...
def validate_args(args, logger):
    if len(args) < 6:
        ...
        logger.error("Usage: python script.py <host> <user> <password> <database> <task_id>")
        sys.exit(1)
...
# Function to get File Name from the database
def get_file_name(db_connection, task_id, logger):
    try:
        cursor = db_connection.cursor()
        get_query = """
        SELECT file_name
        ...
        WHERE id = %s;
        """
        cursor.execute(get_query, (task_id,))
        result = cursor.fetchall()

        cursor.close()
        
        # Check if the result is not empty and extract the file name
        if result:
            file_name = result[0][0]  # Extract file name from the tuple
            logger.info(f"Loaded file name: {file_name} from the database.")
            return file_name
        else:
            logger.error(f"No file name found for task ID {task_id}.")
            return None

    except Exception as e:
        logger.error(f"Error fetching file name from the database: {e}")
        sys.exit(1)

# Function to get Emails List from the database
def get_emails_list(db_connection, file_name, status_flag, logger):
    try:
        # Ensure file_name is a string
        if not isinstance(file_name, str):
            logger.error(f"Invalid file_name type: {type(file_name)}. Expected a string.")
            

        cursor = db_connection.cursor(dictionary=True)
        if status_flag:
            logger.info(f"STATUS FLAG {status_flag}")
            logger.info(f"STATUS FLAG TYPE {type(status_flag)}")
            if status_flag == "1":
                status_condition = "unknown"
            elif status_flag == "2":
                status_condition = "catch all"
            elif status_flag == "3":
                status_condition = ("unknown", "catch all")
            else:
                logger.error(f"Invalid status_flag value: {status_flag}. Exiting...")
                sys.exit(1)  # Exit if status_flag is invalid

            logger.info(f"STATUS CONDITION {status_condition}")
            
            # Construct the query based on the status_flag
            if status_flag == "3":
                get_email_query = """
                SELECT id, email
                FROM email_lists
                WHERE file_name = %s AND status IN (%s, %s);
                """
                cursor.execute(get_email_query, (file_name, status_condition[0], status_condition[1]))
            else:
                get_email_query = """
                SELECT id, email
                FROM email_lists
                ...
                """
                cursor.execute(get_email_query, (file_name, status_condition))
        else:
            get_email_query = """
            SELECT id, email
            FROM email_lists
            WHERE file_name = %s;
            """
            cursor.execute(get_email_query, (file_name,))
        ...
        if not email_list:
            logger.error("No email entries found. Exiting...")
            sys.exit(1)  # Exit if no data is found

        logger.info(f"Loaded {len(email_list)} email entries from the database.")
        return email_list  # Already in desired format due to dictionary=True

    except Exception as e:
        logger.error(f"Error fetching email entries from the database: {e}")
        sys.exit(1)

def sync_status_counts(stop_event, task_id, logger, host, user, password, database):
    try:
        ...
        while not stop_event.is_set():
            ...
            GROUP BY status;
            """
            cursor.execute(count_query, ('17228438908626_csv_file',))
            counts:Any = cursor.fetchall()
            print(counts, task_id ,'--->')
            
            logger.info(f" final_status_count Status counts for Task ID {task_id}: {counts}")
            status_counts = {item['status'].lower(): item['count'] for item in counts}
            print(status_counts)

            # Extracting counts into a lookup dictionary
            update_query = """
                UPDATE tasks
                SET
                    valid = %s,
            ...
            cursor.execute(update_query, (
                status_counts['valid'] if 'valid' in status_counts else 0,
                status_counts['invalid'] if 'invalid' in status_counts else 0,
                status_counts['catch all'] if 'catch all' in status_counts else 0,
                status_counts['disposable'] if 'disposable' in status_counts else 0,
                status_counts['unknown'] if 'unknown' in status_counts else 0,
                task_id
            ))
        ...
        logger.error(f"Error syncing status counts for Task ID {task_id}: {e}")

def final_status_count(db_connection, task_id, logger):
    try:
        cursor = db_connection.cursor(dictionary=True)
        count_query = """
        SELECT status, COUNT(*) as count
        ...
        logger.error(f" final_status_count Error syncing status counts for Task ID {task_id}: {e}")


def start_sync_thread(stop_event, task_id, logger, host, user, password, database):
    logger.info("Starting status counts sync thread...")
    thread = threading.Thread(
        target=sync_status_counts, 
        args=(stop_event, task_id, logger, host, user, password, database),
        daemon=True  # Optional - makes sure thread dies if main program exits
    )
    thread.start()
    return thread
...
def main():
    ...
    database = get_arg(args, 4, "zuhal", logger)
    task_id = get_arg(args, 5, "1", logger)
    api_url = get_arg(args, 6, "", logger)
    status_flag = get_arg(args, 7, "", logger)
    ...
    try: 
        ...
        file_name = get_file_name(db_connection,task_id,logger)
        emails_entries = get_emails_list(db_connection,file_name, status_flag, logger)

        with open('demo_1.json', "r") as file:
            emails_entries = json.load(file)
        
        print(emails_entries)
        # Start the status updater thread after DB connection is established
        background_thread = start_sync_thread(stop_event, task_id, logger, host, user, password, database)
        ...
        if db_connection.is_connected():
            ...
            final_status_count(db_connection, task_id, logger)
            db_connection.close()
            logger.info("Database connection closed.")
        api_response = requests.get(api_url)
    
        if api_response.status_code == 200:
            logger.info(f"API call successful for Task ID {task_id}.")
        else:
            logger.warning(f"API call failed for Task ID {task_id}, Status Code: {api_response.status_code}")
