    ...
    try:
        ...
        for mx in mx_records:
            ...
            try:
                ...
                logger.warning(f"BrokenPipeError Error verifying email {email} with {mx_host}: {e}")
                return None
            except Exception as e:
...
def get_file_name(db_connection,task_id ,logger):
    try:
        ...
        cursor.close()
        logger.info(f"Loaded {len(file_name)} file name from the database.")
        return file_name
    except Exception as e:
...
def get_emails_list(db_connection,file_name, logger):
    try:
...
def main():
    ...
    try: 
        ...
        emails_entries = get_emails_list(db_connection,file_name, logger)
        ...
        # api_url = f"http://127.0.0.1:8000/createJob/{task_id}"
